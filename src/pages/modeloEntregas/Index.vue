<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Modelos de Entrega</h1>
        <p class="text-muted-foreground text-sm mt-1">Gestão dos modelos de entrega de imóveis</p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="border-b border-border bg-muted/50">
            <tr>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground w-24">Código</th>
              ...
              <th class="text-left px-4 py-3 font-medium text-muted-foreground w-32">Operações</th>
            </tr>
          </thead>
          ...
          <tbody>
            ...
            <tr v-else-if="items.length === 0">
              <td colspan="4" class="text-center py-12 text-muted-foreground">
                Nenhum modelo de entrega encontrado.
              </td>
            </tr>
            ...
            >
              <td class="px-4 py-3 font-medium uppercase">{{ modelo.codigo || '-' }}</td>
              <td class="px-4 py-3">{{ modelo.nome || '-' }}</td>
              <td class="px-4 py-3 text-muted-foreground max-w-xl">
                <span class="line-clamp-2">{{ modelo.descricao || '-' }}</span>
              </td>
              <td class="px-4 py-3">
    ...
    <ModeloEntregaViewModal v-model="showViewModal" :modelo-entrega="selected" />
    <ModeloEntregaFormModal
    ...
    <ConfirmDeleteModal
      ...
      entity-label="o modelo de entrega"
...
import ModeloEntregaViewModal from '@/components/modeloEntregas/ModeloEntregaViewModal.vue'
import ModeloEntregaFormModal from '@/components/modeloEntregas/ModeloEntregaFormModal.vue'

const filterNome = ref('')

const { items, paginacao, loading, fetchItems, destroy } = useCrud(modeloEntregaService)
...
function pesquisar(page = 1) {
  fetchItems({
    page,
    search: filterNome.value || undefined,
...
async function confirmDelete() {
  ...
  if (ok) {
    invalidateLookup('lookup:modelo-entregas')
