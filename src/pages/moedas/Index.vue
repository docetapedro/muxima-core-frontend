<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Moedas</h1>
        <p class="text-muted-foreground text-sm mt-1">Gestão de moedas</p>
      </div>
     
    </div>

    <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-3">
      <input v-model="searchTerm" type="text" placeholder="Nome, sigla, descrição..."
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="border-b border-border bg-muted/50">
            <tr>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">Sigla </th>
          ...
          <tbody>
            ...
            <tr v-else v-for="moeda in items" :key="moeda.id"
              class="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
              <td class="px-4 py-3">{{ moeda.sigla }}</td>
              <td class="px-4 py-3">{{ moeda.nome }}</td>
              <td class="px-4 py-3">{{ moeda.descricao }}</td>

              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button @click="openView(moeda)" class="text-primary hover:text-primary/80 transition-colors"
                    title="Ver">
    ...
    <MoedaViewModal v-model="showViewModal" :Moeda="selected" />
    <MoedaFormModal v-model="showCreateModal" mode="create" @success="pesquisar" />
    <MoedaFormModal v-model="showEditModal" mode="edit" :record="selected" @success="pesquisar" />
    <ConfirmDeleteModal v-model="showDeleteModal" entity-label="a moeda" :item-name="selected?.nome"
...
import MoedaViewModal from '@/components/moedas/MoedaViewModal.vue'
import MoedaFormModal from '@/components/moedas/MoedaFormModal.vue'
import ConfirmDeleteModal from '@/components/common/ConfirmDeleteModal.vue'

const searchTerm = ref('')
const { items, paginacao, loading, fetchItems, destroy } = useCrud(moedaservice)
...
function openView(Moeda) {
  selected.value = Moeda
  showViewModal.value = true
}

function openEdit(Moeda) {
  selected.value = Moeda
  showEditModal.value = true
}

function openDelete(Moeda) {
  selected.value = Moeda
  deleteError.value = ''
