<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Tipos de Projecto</h1>
        <p class="text-muted-foreground text-sm mt-1">Gestão de tipos de projecto</p>
      </div>
    ...
    <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-3">
      <input v-model="searchTerm" type="text" placeholder="Nome, descrição..."
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-else-if="items.length === 0">
              <td colspan="3" class="text-center py-12 text-muted-foreground">
                Nenhum tipo de projecto encontrado.
              </td>
            </tr>
            <tr v-else v-for="tipo in items" :key="tipo.id"
              class="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
              <td class="px-4 py-3">{{ tipo.nome || '-' }}</td>
              <td class="px-4 py-3">{{ tipo.descricao || '-' }}</td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button @click="openView(tipo)" class="text-primary hover:text-primary/80 transition-colors"
                    title="Ver">
                    <EyeIcon class="w-4 h-4" />
                  </button>
                  <button @click="openEdit(tipo)" class="text-blue-600 hover:text-blue-700 transition-colors"
                    title="Editar">
                    <Pencil class="w-4 h-4" />
                  </button>
                  <button @click="openDelete(tipo)" class="text-red-600 hover:text-red-700 transition-colors"
                    title="Excluir">
    ...
    <TipoProjectoViewModal v-model="showViewModal" :tipo-projecto="selected" />
    <TipoProjectoFormModal v-model="showCreateModal" mode="create"
      @success="pesquisar(paginacao?.pagina_actual || 1)" />
    <TipoProjectoFormModal v-model="showEditModal" mode="edit" :record="selected"
      @success="pesquisar(paginacao?.pagina_actual || 1)" />
    <ConfirmDeleteModal v-model="showDeleteModal" entity-label="o tipo de projecto" :item-name="selected?.nome"
...
import TipoProjectoViewModal from '@/components/tipoprojectos/TipoProjectoViewModal.vue'
import TipoProjectoFormModal from '@/components/tipoprojectos/TipoProjectoFormModal.vue'
import ConfirmDeleteModal from '@/components/common/ConfirmDeleteModal.vue'

const searchTerm = ref('')
const { items, paginacao, loading, fetchItems, destroy } = useCrud(tipoProjectoService)
