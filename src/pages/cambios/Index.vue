<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Câmbios</h1>
        <p class="text-muted-foreground text-sm mt-1">Gestão de câmbios</p>
      </div>
    ...
    <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-3">
      <input v-model="searchTerm" type="date" placeholder="Data de câmbio..."
        ...
        <table class="w-full text-sm">
          <thead class="border-b border-border bg-muted/50">
            <tr>
              ...
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">USD</th>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">EUR</th>
            ...
            <tr v-else v-for="cambio in items" :key="cambio.id"
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button @click="openView(cambio)" class="text-primary hover:text-primary/80 transition-colors"
                    title="Ver">
                    <EyeIcon class="w-4 h-4" />
                  </button>
                  <button @click="openEdit(cambio)" class="text-blue-600 hover:text-blue-700 transition-colors"
                    title="Editar">
                    <Pencil class="w-4 h-4" />
                  </button>
                  <button @click="openDelete(cambio)" class="text-red-600 hover:text-red-700 transition-colors"
                    title="Excluir">
...
const { items, paginacao, loading, fetchItems, destroy } = useCrud(cambioservice)
...
function openView(Cambio) {
  selected.value = Cambio
