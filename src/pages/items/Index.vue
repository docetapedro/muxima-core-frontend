  ...
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground text-sm mt-1">Gestão de items</p>
      </div>
    ...
    <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-3">
      <input v-model="searchTerm" type="text" placeholder="Código, nome..."
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="border-b border-border bg-muted/50">
            <tr>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">Código</th>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">Nome</th>
              <th class="text-left px-4 py-3 font-medium text-muted-foreground">Dias após vencimento</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="item in items" :key="item.id"
              ...
              <td class="px-4 py-3">{{ item.dias_apos_vencimento ?? '-' }}</td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button @click="openView(item)" class="text-primary hover:text-primary/80 transition-colors"
                    title="Ver">
                    <EyeIcon class="w-4 h-4" />
                  </button>
                  <button @click="openEdit(item)"
                    class="text-blue-600 hover:text-blue-700 transition-colors"
                    :title="isItemProtegido(item) ? 'Item protegido — só permite editar Dias após vencimento' : 'Editar'">
                    <Pencil class="w-4 h-4" />
                  </button>
                  <button v-if="!isItemProtegido(item)" @click="openDelete(item)"
                    class="text-red-600 hover:text-red-700 transition-colors" title="Excluir">
                    <Trash2 class="w-4 h-4" />
                  </button>
                  <Lock v-if="isItemProtegido(item)" class="w-4 h-4 text-muted-foreground"
                    title="Item protegido — não pode ser removido" />
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <ItemViewModal v-model="showViewModal" :item="selected" />
    <ItemFormModal v-model="showCreateModal" mode="create" @success="pesquisar(paginacao?.pagina_actual || 1)" />
    <ItemFormModal v-model="showEditModal" mode="edit" :record="selected"
      @success="pesquisar(paginacao?.pagina_actual || 1)" />
    <ConfirmDeleteModal v-model="showDeleteModal" entity-label="o item" :item-name="selected?.nome"
...
import ItemViewModal from '@/components/items/ItemViewModal.vue'
import ItemFormModal from '@/components/items/ItemFormModal.vue'
import ConfirmDeleteModal from '@/components/common/ConfirmDeleteModal.vue'
import { isItemProtegido } from '@/utils/itemForm'

const searchTerm = ref('')
const { items, paginacao, loading, fetchItems, destroy } = useCrud(itemService)
...
function openDelete(item) {
  if (isItemProtegido(item)) {
    toast.error('Este item está protegido e não pode ser removido.')
    return
  }
  selected.value = item
  deleteError.value = ''
...
async function confirmDelete() {
  ...
  if (isItemProtegido(selected.value)) {
    deleteError.value = 'Este item está protegido e não pode ser removido.'
    return
  }
  deleting.value = true
  deleteError.value = ''
