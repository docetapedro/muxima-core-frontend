<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold">Tipo de Imóveis</h1>
        <p class="text-muted-foreground text-sm mt-1">Gestão de tipos de imóveis</p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr
              v-else
              v-for="tipoImovel in items"
              :key="tipoImovel.id"
              class="border-b border-border last:border-0 hover:bg-muted/30 transition-colors"
            >
              <td class="px-4 py-3">{{ tipoImovel.nome || '-' }}</td>
              <td class="px-4 py-3">{{ tipoImovel.descricao || '-' }}</td>

              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button
                    @click="openView(tipoImovel)"
    ...
    <TipoImovelViewModal v-model="showViewModal" :modelo="selected" />
    <TipoImovelFormModal
...
import TipoImovelViewModal from '@/components/tipoimoveis/TipoImovelViewModal.vue'
import TipoImovelFormModal from '@/components/tipoimoveis/TipoImovelFormModal.vue'

const filterNome = ref('')

const { items, paginacao, loading, fetchItems, destroy } = useCrud(tipoImovelService)
...
function openView(tipoImovel) {
  selected.value = tipoImovel
  showViewModal.value = true
}

function openEdit(tipoImovel) {
  selected.value = tipoImovel
  showEditModal.value = true
}

function openDelete(tipoImovel) {
  selected.value = tipoImovel
  deleteError.value = ''
