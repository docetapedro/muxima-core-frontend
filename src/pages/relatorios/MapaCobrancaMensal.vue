<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-2xl font-bold">Mapa de Cobrança Mensal</h1>
      <p class="text-muted-foreground text-sm mt-1">
        Indique o ano (e opcionalmente um condomínio) e gere o PDF com o mapa de cobrança mensal.
      </p>
    </div>
    ...
    <div class="bg-card border border-border rounded-xl p-6 space-y-4 max-w-2xl">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input v-model.number="ano" type="number" min="2000" max="2100" step="1"
...
const validAno = computed(() => Number.isInteger(ano.value) && ano.value >= 2000 && ano.value <= 2100)
const canGenerate = computed(() => validAno.value && !!selectedCondominioId.value)
...
async function gerar(mode) {
  ...
  error.value = ''
  try {
    const response = await relatorioService.mapaCobrancaMensalPdf(ano.value, selectedCondominioId.value)
    const c = condominios.value.find((x) => x.id === selectedCondominioId.value)
    const slug = c ? `-${slugify(c.nome_abreviado || c.nome)}` : ''
    const fileName = `mapa-cobranca-mensal-${ano.value}${slug}`
    if (mode === 'open') {
    ...
    console.error(e)
    error.value = await extractBlobErrorMessage(e, 'Falha ao gerar o mapa de cobrança.')
  } finally {
