<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="bg-card border border-border rounded-xl p-4 flex flex-wrap gap-3">
      <!-- Input geral = referencia -->
      <input v-model="searchTerm" type="text" placeholder="Referência..."
      ...
      <!-- Condomínio -->
      <select v-model.number="filters.condominio"
      ...
      <!-- Tipologia -->
      <select v-model.number="filters.tipologia"
        class="min-w-48 px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <!-- Tipo Imóvel -->
      <select v-model.number="filters.tipo_imovel"
        class="min-w-48 px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <!-- Modelo -->
      <select v-model.number="filters.modelo"
        class="min-w-48 px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <button @click="pesquisar(1)" :disabled="loading"
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-else v-for="imovel in items" :key="imovel.id"
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <button @click="openView(imovel)" class="text-primary hover:text-primary/80 transition-colors"
                    title="Ver">
                    <EyeIcon class="w-4 h-4" />
                  </button>
                  <button @click="openEdit(imovel)" class="text-blue-600 hover:text-blue-700 transition-colors"
                    title="Editar">
                    <Pencil class="w-4 h-4" />
                  </button>
                  <button @click="openDelete(imovel)" class="text-red-600 hover:text-red-700 transition-colors"
                    title="Excluir">
    ...
    <ImovelFormModal v-model="showEditModal" mode="edit" :record="selected"
      @success="pesquisar(paginacao?.pagina_actual || 1)" />

    <ConfirmDeleteModal v-model="showDeleteModal" entity-label="o imóvel" :item-name="deleteLabel" :loading="deleting"
      :error="deleteError" @confirm="confirmDelete" />
  </div>
</template>
...
async function loadOptions() {
  // normaliza para “array de itens” o mais possível, independentemente da API
  const toList = (res) => res?.data?.dados?.items ?? res?.data?.dados ?? res?.data ?? []

  try {
    const [cRes, tRes, tiRes, mRes] = await Promise.all([
      condominioService.listar({ page: 1 }),
      tipologiaService.listar({ page: 1 }),
      tipoImovelService.listar({ page: 1 }),
      modeloService.listar({ page: 1 })
    ])

    condominioOptions.value = toList(cRes) ?? []
    tipologiasOptions.value = toList(tRes) ?? []
    tipoImovelOptions.value = toList(tiRes) ?? []
    modeloOptions.value = toList(mRes) ?? []
  } catch (e) {
    // deixa a lista funcionar mesmo se falhar carregar opções
    console.error(e)
