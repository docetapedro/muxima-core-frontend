  ...
  <div class="min-h-screen flex items-center justify-center bg-muted/30 p-4">
    <div class="w-full max-w-sm">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold">Muxima</h1>
        <p class="text-muted-foreground text-sm mt-1">Sistema de Gestão de Imobiliária</p>
      </div>

      <!-- Card -->
      <div class="bg-card border border-border rounded-xl p-6 shadow-sm">
        <h2 class="text-lg font-semibold mb-1">Iniciar sessão</h2>
        <p class="text-muted-foreground text-sm mb-6">Introduza as suas credenciais</p>

        <form @submit.prevent="handleLogin" class="space-y-4">
          <!-- Login field -->
          <div class="space-y-1.5">
            <label class="text-sm font-medium">E-mail ou Utilizador</label>
            <input
              v-model="form.login"
              type="text"
          ...
          <!-- Password field -->
          <div class="space-y-1.5">
            <label class="text-sm font-medium">Palavra-passe</label>
            <div class="relative">
              <input
                v-model="form.password"
                :type="showPassword ? 'text' : 'password'"
                placeholder="••••••••"
                class="w-full px-3 py-2 pr-10 rounded-md border border-input bg-background text-sm
                       focus:outline-none focus:ring-2 focus:ring-ring transition-colors
                       placeholder:text-muted-foreground"
                :disabled="auth.loading"
                required
          ...
          <button
            type="submit"
            :disabled="auth.loading"
            class="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-md
