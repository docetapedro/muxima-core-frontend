  ...
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground text-sm mt-1">Gestão de projectos</p>
      </div>
    ...
    <ProjectoViewModal v-model="showViewModal" :projecto="selected" />
    <ProjectoFormModal
...
import ProjectoViewModal from '@/components/projectos/ProjectoViewModal.vue'
import ProjectoFormModal from '@/components/projectos/ProjectoFormModal.vue'

const filterNome = ref('')

const { items, paginacao, loading, fetchItems, destroy } = useCrud(projectoService)
...
function openView(projecto) {
  selected.value = projecto
  showViewModal.value = true
}

function openEdit(projecto) {
  selected.value = projecto
  showEditModal.value = true
}

function openDelete(projecto) {
  selected.value = projecto
  deleteError.value = ''
