<template>
  <ModalWrapper v-model="visible" :title="isEdit ? 'Editar Cliente' : 'Nova Entidade'" width="large">
    <form id="cliente-form" @submit.prevent="submit" class="space-y-4">
      <EntidadeFormFields v-model="formData" :validation-errors="validationErrors" />
...
const formData = ref(emptyEntidade())
...
watch(() => props.modelValue, (open) => {
  ...
  formData.value = props.record ? entidadeFromRecord(props.record) : emptyEntidade()
})
