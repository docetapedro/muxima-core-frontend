<template>
  <ModalWrapper v-model="visible" :title="isEdit ? 'Editar Moeda' : 'Nova Moeda'" width="large">
    <form id="cliente-form" @submit.prevent="submit" class="space-y-4">
      <MoedaFormFields v-model="formData" :validation-errors="validationErrors" />
...
watch(() => props.modelValue, (open) => {
  ...
  formData.value = props.record ? moedaFromRecord(props.record) : emptyMoeda()
})
